...
class Tournament {
  ...
  static async findAll() {
    const sql =
      'SELECT * FROM tournaments WHERE status = "pending" OR status = "ongoing"';
  ...
  static async getPlayers(id) {
    const sql = `SELECT wallet_address FROM players WHERE tournament_id = ?`;
    const [rows] = await db.execute(sql, [id]);
    return rows;
  }

  static async getTournamentDetails(id) {
    const sql = `SELECT entry_price, creator_wallet_address FROM tournaments WHERE id = ?`;
    const [rows] = await db.execute(sql, [id]);
    return rows[0];
  }
